...
class ImprovedProcessor:
    ...
    def find_header_row_and_columns(self, df):
        ...
        for row_idx in range(min(25, len(df))):
            ...
            for col_idx, value in enumerate(row):
                if pd.notna(value):
                    ...
                    elif 'セリフ' in value_str or 'せりふ' in value_str:
                        dialogue_col = col_idx
            
            # 両方見つかった場合
            if character_col is not None and dialogue_col is not None:
                return row_idx, character_col, dialogue_col
            
            # キャラクターのみ見つかった場合、隣接列をセリフとして検索
            if character_col is not None:
                for offset in [1, -1, 2]:  # 右隣、左隣、右から2番目
                    dialogue_candidate = character_col + offset
                    if 0 <= dialogue_candidate < len(row):
                        candidate_value = row.iloc[dialogue_candidate]
                        if pd.notna(candidate_value):
                            candidate_str = str(candidate_value).strip().lower()
                            if 'セリフ' in candidate_str or 'せりふ' in candidate_str:
                                return row_idx, character_col, dialogue_candidate
        
        # パターン2: 構造的検出（No.列の存在から推測）
        for row_idx in range(min(15, len(df))):
            ...
            if no_col is not None:
                # No.列から推測されるキャラクター・セリフ列
                character_col = no_col + 1  # No.の次
                dialogue_col = no_col + 2   # No.の次の次
                
                # 列が存在するかチェック
                if character_col < len(row) and dialogue_col < len(row):
                    return row_idx, character_col, dialogue_col
        
        # パターン3: 固定位置検出（他のスプレッドシートから学習した構造）
        # 行4周辺をヘッダーとして、列2をキャラクター、列3をセリフとして試行
        for header_row in [4, 3, 5]:
            if header_row < len(df):
                character_col = 2  # 3列目
                dialogue_col = 3   # 4列目
                
                # データが存在するかチェック
                if character_col < len(df.columns) and dialogue_col < len(df.columns):
                    # この構造でデータが存在するかサンプルチェック
                    sample_found = False
                    for sample_row in range(header_row + 1, min(header_row + 10, len(df))):
                        if sample_row < len(df):
                            char_val = df.iloc[sample_row, character_col] if character_col < len(df.columns) else None
                            dial_val = df.iloc[sample_row, dialogue_col] if dialogue_col < len(df.columns) else None
                            
                            if pd.notna(char_val) and pd.notna(dial_val):
                                char_str = str(char_val).strip()
                                dial_str = str(dial_val).strip()
                                
                                # キャラクター名らしきパターン（サンサン、くもりん等）
                                if len(char_str) > 0 and len(dial_str) > 0 and len(char_str) < 20:
                                    sample_found = True
                                    break
                    
                    if sample_found:
                        return header_row, character_col, dialogue_col
        ...
        try:
            ...
            for idx in range(header_row + 1, len(df)):
                ...
                if character_col < len(row) and dialogue_col < len(row):
                    ...
                    if pd.notna(character) and pd.notna(dialogue):
                        ...
                        if character_str and dialogue_str and len(character_str) < 100:  # 異常に長いキャラクター名を除外
            ...
            for item in dialogue_data:
                cursor.execute("""
                    ...
                    (script_id, character_name, dialogue_text) 
                    VALUES (?, ?, ?)
    ...
    def run(self):
        ...
        self.log_message("================================================================================")
        self.log_message(f"改良版URL処理完了: {success_count}/{total_scripts}件成功")
        self.log_message("================================================================================")

if __name__ == "__main__":
    processor = ImprovedProcessor("/Users/mitsuruono/sunsun_script_search/sunsun_script_database/youtube_search_complete_all.db")
    processor.run()